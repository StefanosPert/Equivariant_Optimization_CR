...
def get_args_parser():
    ...
    parser.add_argument('--model-ema-decay', type=float, default=0.999, help='')
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        if epoch<args.epochs/2.0:
            ...
            mix_coef=1-2*(epoch-args.epochs/2.0)/args.epochs
        
        ## simpler schedule
        #mix_coef=1-epoch/args.epochs
        ...
        train_err, train_loss = train_one_epoch(args=args, model=model, criterion=criterion,
            ...
            print_freq=args.print_freq, logger=_log,mixing_coef=mix_coef)
    ...
    # evaluate on the whole testing set using the best validation model
    best_val_path = os.path.join(args.output_dir, 'best_val_epoch.pth.tar')
    if os.path.exists(best_val_path):
        best_val_state = torch.load(best_val_path, map_location=device)
        model.load_state_dict(best_val_state['state_dict'])
        _log.info('Loaded best validation model from epoch {}'.format(best_metrics['val_epoch']))
    else:
        _log.info('No best validation checkpoint found, using final model')
    test_err, test_loss = evaluate(args=args, model=model, criterion=criterion,
        data_loader=test_loader, device=device,
        print_freq=args.print_freq, logger=_log, print_progress=True, max_iter=-1)
...
def train_one_epoch(args, 
    ...
    for step, data in enumerate(data_loader):
        ...
        random_matrix = torch.randn(3, 3, device=device)
        ...
        #print(Q.shape)
        rotated_pos = data.pos @ Q
        mean_pos = scatter(rotated_pos, data.batch, dim=0, reduce='mean')
        rotated_pos = rotated_pos - mean_pos[data.batch]

        pred_y, pred_dy,reg = model(node_atom=data.z, pos=rotated_pos, batch=data.batch,return_reg=True,mixing_coef=mixing_coef)
        pred_dy = pred_dy @ Q.T
        loss_e = criterion(pred_y, ((data.y - task_mean) / task_std))
        loss_f = criterion(pred_dy, (data.dy / task_std))
        loss = args.energy_weight * loss_e + args.force_weight * loss_f +0.01*reg
...
def evaluate(args, 
    ...
    with torch.no_grad():
        ...
        for step, data in enumerate(data_loader):
            ...
            pred_y, pred_dy = model(node_atom=data.z, pos=rotated_pos, batch=data.batch)
