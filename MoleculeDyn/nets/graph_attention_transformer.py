...
def get_lie_algebra_generators_e3nn(irreps):
    """
    Compute Lie algebra generators using e3nn's built-in functions.
    """
    irreps = o3.Irreps(irreps)
    #print("Attempt generators") 
    # Build block-diagonal generator matrices for all irreps
    generators = []
    for mul, ir in irreps:
        # Get generators for this irrep (returns 3 matrices of shape [2l+1, 2l+1])
        gen = so3_generators(ir.l)  # shape: (3, 2l+1, 2l+1)
    ...
    # Block-diagonalize
    dim = irreps.dim
    A = torch.zeros(3, dim, dim)
    idx = 0
    for mul, ir in irreps:
        d = ir.dim
        gen = so3_generators(ir.l)
        for _ in range(mul):
            A[:, idx:idx+d, idx:idx+d] = gen
            idx += d
    #print("Succed")
    return A


def get_wigner_d_matrix(irreps, angle_deg, axis_idx):
    """
    Compute block-diagonal Wigner D matrix for a rotation of `angle_deg`
    degrees around axis `axis_idx` (0=x, 1=y, 2=z).
    """
    irreps = o3.Irreps(irreps)
    ...
    for mul, ir in irreps:
        d = ir.dim
        wD = o3.wigner_D(ir.l, alpha, beta, gamma)
        for _ in range(mul):
            D[idx:idx+d, idx:idx+d] = wD
            idx += d
    return D
...
class FeedForwardNetworkRel(torch.nn.Module):
    '''
        Typical feedforward network used in transformers.
    ...
    def __init__(self,
        ...
        # Scale down weights by 0.05
        with torch.no_grad():
            #self.fc1.weight.mul_(0.05)
            #self.fc1.bias.mul_(0.05)
...
class TransBlock(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.concat_irrep = self.irreps_node_input + self.irreps_node_attr + o3.Irreps("1x1o")

        # Pre-compute Wigner D matrices for 5-degree rotations around x, y, z
        D_in_list = []
        D_out_list = []
        for axis_idx in range(3):
            D_in_list.append(get_wigner_d_matrix(self.concat_irrep, 5.0, axis_idx))
            D_out_list.append(get_wigner_d_matrix(self.irreps_node_output, 5.0, axis_idx))
        self.register_buffer('D_in', torch.stack(D_in_list))    # (3, d_in, d_in)
        self.register_buffer('D_out', torch.stack(D_out_list))   # (3, d_out, d_out)

        self.ffn_relax = FeedForwardNetworkRel(self.irreps_node_input.dim + self.irreps_node_attr.dim + 3, out_features=irreps_node_output.dim)
    ...
    def forward(self, node_input, node_attr, edge_src, edge_dst, edge_attr, edge_scalars, 
        batch,return_reg=False,mixing_coef=1,pos=None,**kwargs):
        ...
        #print(torch.cat([node_features,node_attr,pos],dim=-1).shape)
        relax_features=self.ffn_relax(torch.cat([node_features,node_attr,pos],dim=-1))

        if return_reg:
            det_feat = torch.cat([node_features, node_attr, pos], dim=-1).detach()
            rotated_input = torch.einsum('ni,aji->anj', det_feat, self.D_in)  # (3, N, d_in)
            rotated_output = self.ffn_relax(rotated_input)  # (3, N, d_out)

            rotated_back = torch.einsum('ani,aij->anj', rotated_output, self.D_out)
            reg = torch.mean(torch.norm(rotated_back - relax_features.unsqueeze(0), dim=-1))
        else:
            reg = 0
        node_features = self.ffn(node_features, node_attr)
        
        node_features=node_features+mixing_coef*relax_features
...
class GraphAttentionTransformer(torch.nn.Module):
    ...
    def forward(self, f_in, pos, batch, node_atom,return_reg=False, **kwargs) -> torch.Tensor:
        ...
        for blk in self.blocks:
            node_features,reg = blk(node_input=node_features, node_attr=node_attr, 
                edge_src=edge_src, edge_dst=edge_dst, edge_attr=edge_sh, 
                edge_scalars=edge_length_embedding, 
                batch=batch,return_reg=return_reg)
            total_reg+=reg
