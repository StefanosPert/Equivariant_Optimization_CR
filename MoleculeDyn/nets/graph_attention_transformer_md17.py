...
class GraphAttentionTransformerMD17(torch.nn.Module):
    ...
    @torch.enable_grad()
    def forward(self, node_atom, pos, batch,return_reg=False,mixing_coef=1):
        ...
        for blk in self.blocks:
            node_features,reg = blk(node_input=node_features, node_attr=node_attr,edge_src=edge_src, edge_dst=edge_dst,
                                    edge_attr=edge_sh, edge_scalars=edge_length_embedding, batch=batch,return_reg=return_reg,pos=pos,mixign_coef=mixing_coef)
            total_reg+=reg
        ...
        if return_reg:
            return energy,forces,total_reg
